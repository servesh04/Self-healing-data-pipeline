...
// Shared by RunChat (per-run) and GlobalChat (Overview) — same UI shape,
// same message rendering, same mono-highlighting; only the suggested chips
// and the send function differ per scope. A blank input invites the one
// question that produces a bad answer — the chips are required, not
// decoration (see the feature spec).
...
export default function ChatPanel({ suggested, onSend, placeholder = 'Ask a question…' }) {
  ...
  async function send(text) {
    ...
    setError(null)
    try {
      const { reply } = await onSend(next.slice(-MAX_SENT_MESSAGES))
